...
function BariKoiLiveReligionMap({
  ...
  // Zoom out to show user location and nearby religious places when map is compact
  useEffect(() => {
    const map = mapRef.current;
  ...
  return (
    <AppLayout noPad={true}>
      <div className="w-full h-[calc(100dvh-4rem)] lg:h-[calc(100vh)] flex flex-col overflow-hidden bg-[#FAFAFA]">
        ...
        {/* ── DUAL-STATE BARIKOI LIVE MAP (FIXED HEIGHT BY DEFAULT - COLLAPSES TO 0 ON FULL SCREEN) ── */}
        <div
          id="religion-map-section"
        ...
        {/* ── MAIN DIRECTORY CONTENT (UPORE / ON TOP - FIXED MAP, FREE CARD SCROLL, PULL HANDLE FOR FULL SCREEN) ── */}
        <div
          ref={cardListRef}
        ...
        >
          {/* Sticky Header: Pull handle ("black shadow line") + Counter Toggle Boxes */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-0 sm:gap-5 items-stretch pt-3 px-4 sm:px-0 mb-6">

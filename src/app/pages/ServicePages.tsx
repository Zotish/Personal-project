...
const jobs = [
  { id: 1, title: "Software Engineer", company: "Tech Solutions NYC", location: "New York, NY (Hybrid)", salary: "$90K–$130K/yr", type: "Full-time", visa: "H-1B Sponsor", logo: "💻", posted: "2h ago", description: "Looking for a full-stack engineer with 3+ years experience. H-1B sponsorship available for qualified candidates.", skills: ["React", "Node.js", "AWS"], rating: 4.3, reviews: 234 },
  { id: 2, title: "Registered Nurse (RN)", company: "Elmhurst Hospital Center", location: "Queens, NY", salary: "$75K–$95K/yr", type: "Full-time", visa: "EB-3 Sponsor", logo: "🏥", posted: "5h ago", description: "Seeking experienced RN for ICU department. International nurses welcome. Visa sponsorship for EB-3 available.", skills: ["ICU", "Patient Care", "NCLEX"], rating: 4.1, reviews: 567 },
  { id: 3, title: "Restaurant Manager", company: "Dhaka Kitchen", location: "Jackson Heights, NY", salary: "$50K–$65K/yr", type: "Full-time", visa: "Work Permit OK", logo: "🍽️", posted: "1d ago", description: "Bangladeshi restaurant seeking experienced manager. Bengali/English bilingual preferred.", skills: ["Management", "Bengali", "Hospitality"], rating: 4.6, reviews: 89 },
  { id: 4, title: "ESL Teacher", company: "NYC Public Schools", location: "Multiple Locations, NY", salary: "$58K–$85K/yr", type: "Full-time", visa: "No Sponsorship", logo: "📚", posted: "2d ago", description: "Teaching English as a Second Language to adult immigrants. TESOL certification required.", skills: ["TESOL", "ESL", "Teaching"], rating: 4.4, reviews: 412 },
  { id: 5, title: "Delivery Driver", company: "Amazon Logistics", location: "Brooklyn, NY", salary: "$18–$22/hr", type: "Part-time", visa: "Work Auth Required", logo: "📦", posted: "3d ago", description: "No experience needed. Valid driver's license required. Work authorization (EAD or Green Card) needed.", skills: ["Driving", "Navigation"], rating: 3.8, reviews: 1203 },
];
      ...
      <div className="max-w-2xl mx-auto">
        <div className="p-4 sm:p-5 bg-white border-b border-border">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold text-foreground" style={{ fontFamily: "var(--font-display)" }}>Find Jobs</h1>
          </div>
          <p className="text-sm text-muted-foreground mb-4">Immigrant-friendly employers who offer visa sponsorship and welcome new arrivals</p>
          <div className="relative mb-3">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input type="text" placeholder="Search jobs, companies, skills..." className="w-full pl-10 pr-4 py-2.5 bg-input-background rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-primary transition" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {["all", "H-1B Sponsor", "EB-3 Sponsor", "No Visa Needed", "Part-time"].map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all ${filter === f ? "bg-primary text-white" : "bg-secondary border border-border text-muted-foreground hover:text-primary"}`}>
                {f}
              </button>
            ))}
          </div>
        ...
        <div className="p-4 space-y-3">
          {jobs.map(job => (
            <div key={job.id} className="bg-white rounded-2xl border border-border p-4 hover:shadow-sm transition-all cursor-pointer group">
              <div className="flex gap-3">
                <div className="w-11 h-11 rounded-xl bg-secondary flex items-center justify-center text-2xl flex-shrink-0">{job.logo}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">{job.title}</div>
                      <div className="text-xs text-muted-foreground">{job.company}</div>
                    </div>
                    <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-medium flex-shrink-0 whitespace-nowrap">{job.visa}</span>
                  </div>
                  ...
                  <div className="flex gap-1.5 mt-2 flex-wrap">
                    {job.skills.map(s => <span key={s} className="text-xs bg-secondary text-muted-foreground px-2 py-0.5 rounded-full">{s}</span>)}
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Star className="w-3 h-3 fill-amber-400 text-amber-400" />{job.rating} · {job.reviews} reviews · {job.posted}
                    </div>
                    <button className="px-3 py-1.5 rounded-xl bg-primary text-white text-xs font-semibold hover:opacity-90 transition">Apply</button>
                  </div>
                </div>
              </div>
            </div>

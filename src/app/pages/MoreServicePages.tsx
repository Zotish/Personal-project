...
// ─── Religious Institution Finder ──────────────────────────────────────────

const religiousPlaces = [
  {
    id: 1, name: "Masjid At-Taqwa", type: "Mosque", distance: "0.3 mi", rating: 4.8, reviews: 342,
    open: true, openUntil: "9:00 PM", address: "1266 Bedford Ave, Brooklyn, NY 11216", phone: "+1 (718) 622-0800",
    languages: ["Arabic", "Bengali", "Wolof", "English"], emoji: "🕌", prayerTimes: true,
    features: ["Jumu'ah", "Islamic Classes", "Food Pantry", "Women's Section"],
    image: "https://images.unsplash.com/photo-1564769625905-50e93615e769?w=400&h=200&fit=crop",
    desc: "Historic mosque serving Brooklyn's Muslim community. Bengali and Arabic services. Jumu'ah at 1:15 PM."
  },
  {
    id: 2, name: "Hindu Temple Society of North America", type: "Hindu Temple", distance: "2.1 mi", rating: 4.7, reviews: 891,
    open: true, openUntil: "9:00 PM", address: "45-57 Bowne St, Flushing, NY 11355", phone: "+1 (718) 460-8484",
    languages: ["Hindi", "Tamil", "Bengali", "English"], emoji: "🛕", prayerTimes: false,
    features: ["Daily Puja", "Cultural Events", "Sanskrit Classes", "Community Hall"],
    image: "https://images.unsplash.com/photo-1548013146-72479768bada?w=400&h=200&fit=crop",
    desc: "The oldest Hindu temple in New York. All major Hindu festivals celebrated. Prasad distribution daily."
  },
  {
    id: 3, name: "St. Teresa's Catholic Church", type: "Church", distance: "0.9 mi", rating: 4.5, reviews: 456,
    open: true, openUntil: "7:00 PM", address: "450 Baltic St, Brooklyn, NY 11217", phone: "+1 (718) 624-2176",
    languages: ["English", "Spanish", "Haitian Creole", "Polish"], emoji: "⛪", prayerTimes: false,
    features: ["Spanish Mass", "Immigrant Outreach", "Food Bank", "ESL Classes"],
    image: "https://images.unsplash.com/photo-1548252539-48eb4ad5ae4d?w=400&h=200&fit=crop",
    desc: "Welcoming parish with strong immigrant outreach programs. Spanish mass every Sunday at 12 PM."
  },
  {
    id: 4, name: "Sikh Cultural Center", type: "Gurdwara", distance: "3.4 mi", rating: 4.9, reviews: 234,
    open: true, openUntil: "9:00 PM", address: "95-30 118th St, Richmond Hill, NY 11419", phone: "+1 (718) 846-8900",
    languages: ["Punjabi", "Hindi", "English"], emoji: "🏛️", prayerTimes: false,
    features: ["Langar (Free Meals)", "Kirtan", "Volunteer Opportunities", "Community Help"],
    image: "https://images.unsplash.com/photo-1590080876212-efba3f1e5e20?w=400&h=200&fit=crop",
    desc: "Sikh temple offering free daily meals (Langar) to all, regardless of faith or background. Community help available."
  },
];
...
const religionFilters = [
  ...
  { id: "gurdwara", label: "Gurdwara", emoji: "🏛️" },
  { id: "synagogue", label: "Synagogue", emoji: "✡️" },
      ...
      <div className="max-w-2xl mx-auto">
        <div className="p-4 bg-white border-b border-border">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold text-foreground" style={{ fontFamily: "var(--font-display)" }}>Find Religious Places</h1>
          </div>
          <p className="text-sm text-muted-foreground mb-4">Mosques, temples, churches, gurdwaras, and more near your location.</p>
          <div className="relative mb-3">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input type="text" placeholder="Search by religion, language, name..." className="w-full pl-10 pr-4 py-2.5 bg-input-background rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-ring transition" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {religionFilters.map(({ id, label, emoji }) => (
              <button key={id} onClick={() => setActiveFilter(id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all ${activeFilter === id ? "bg-primary text-white" : "bg-white border border-border text-muted-foreground hover:text-primary"}`}>
                <span>{emoji}</span>{label}
              </button>
            ))}
          </div>
          ...
          {religiousPlaces.map(place => (
            <div key={place.id} className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-md transition-all cursor-pointer group">
              <div className="relative h-32 overflow-hidden">
                ...
                <div className="absolute top-2 left-2 bg-white/90 text-foreground text-xs font-semibold px-2 py-1 rounded-full flex items-center gap-1">
                  <span>{place.emoji}</span>{place.type}
                </div>
                <div className={`absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${place.open ? "bg-emerald-500 text-white" : "bg-red-500 text-white"}`}>
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1"><MapPin className="w-3 h-3" />{place.distance}</div>
                      <div className="flex items-center gap-1"><Star className="w-3 h-3 fill-amber-400 text-amber-400" />{place.rating} ({place.reviews})</div>
                    </div>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mb-3 leading-relaxed">{place.desc}</p>
                <div className="mb-3">
                  <div className="text-xs font-medium text-foreground mb-1">Features</div>
                  <div className="flex gap-1.5 flex-wrap">
                    {place.features.map(f => <span key={f} className="text-xs bg-orange-50 text-orange-700 px-2 py-0.5 rounded-full">{f}</span>)}
                  </div>
                </div>
                <div className="mb-3">
                  <div className="text-xs font-medium text-foreground mb-1">Languages</div>
                  <div className="flex gap-1.5 flex-wrap">
                    {place.languages.map(l => <span key={l} className="text-xs bg-blue-50 text-primary px-2 py-0.5 rounded-full">{l}</span>)}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-primary text-white text-xs font-semibold hover:opacity-90 transition">

      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            { icon: Wind,        label: t("weather_wind"),     value: `${weather.windSpeed} km/h`, color: "text-emerald-600"},
...
function LocalBanner() {
  return (
    <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 p-3 sm:p-4 flex items-center gap-3 mb-1">
      <MapPin className="w-7 h-7 sm:w-8 sm:h-8 text-emerald-600 flex-shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="text-sm font-semibold text-emerald-900">Posts near you</div>
        <div className="text-xs text-emerald-600">📍 Showing posts from Queens, Brooklyn & NYC area</div>
      </div>
      <button className="text-xs text-emerald-600 font-medium hover:underline flex-shrink-0">Change</button>
    </div>
  );
}
  ...
  const trigger = variant === "desktop" ? (
    ...
    <button
      ...
      className={`w-full h-11 rounded-2xl flex items-center justify-center border transition-all shadow-sm ${
        open ? "bg-secondary border-border shadow-md" : "bg-white border-border hover:shadow-md"
      }`}
    >

...
function InteractiveServiceMap({
  ...
  // Zoom out to show user location and nearby markers when in compact/scrolled mode
  useEffect(() => {
    const map = mapInstanceRef.current;
    if (!map || !mapLoaded || directionItem || selectedItem) return;

    if (isScrolled) {
      // Map is in compact mode: Zoom out so user location and nearby pins are in view
      const bkoigl = (window as any).bkoigl;
      if (bkoigl) {
        const bounds = new bkoigl.LngLatBounds();
        bounds.extend([userCoords[1], userCoords[0]]);
        (items || []).slice(0, 4).forEach(it => {
          if (it.lng && it.lat) bounds.extend([it.lng, it.lat]);
        });
      ...
      // Expanded full size: Zoom back in centered on user location
      if (map.flyTo) {
        map.flyTo({
          center: [userCoords[1], userCoords[0]],
          zoom: 14.5,
  ...
  }, [isScrolled, userCoords, items, mapLoaded, directionItem, selectedItem]);
  ...
  return (
    <AppLayout noPad={true}>
      <div className="w-full h-[calc(100dvh-4rem)] lg:h-[calc(100vh)] flex flex-col overflow-hidden bg-[#FAFAFA]">
        ...
        {/* ── MAIN DIRECTORY LISTINGS (UPORE / ON TOP - FIXED MAP, FREE CARD SCROLL, PULL HANDLE FOR FULL SCREEN) ─────────── */}
        <div
          ref={cardListRef}
        ...
        >
          {/* Sticky Header: Pull handle ("black shadow line") + Nearby / Total filters */}

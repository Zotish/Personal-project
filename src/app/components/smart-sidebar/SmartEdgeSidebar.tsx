...
import {
  X, Minus, Maximize2, Minimize2, Search, Move,
  ...
  Share2, Heart, Pause, Forward, SkipForward, ArrowUpRight
} from "lucide-react";
...
// Full Device Installed Apps for Selection
...
export function SmartEdgeSidebar() {
  ...
  const [windowMode, setWindowMode] = useState<"split-50" | "floating" | "maximized">("split-50");
  const [splitHeightPercent, setSplitHeightPercent] = useState<number>(50);
  ...
  // Freeform Position & Dimensions (px) for dragging anywhere and resizing
  ...
  // Initialize or reset position & size when app opens or mode changes
  const resetToBottomSplit = useCallback(() => {
    if (typeof window === "undefined") return;
    const isSmall = window.innerWidth < 768;
    const w = isSmall ? window.innerWidth - 20 : Math.min(500, window.innerWidth - 40);
    const h = Math.round(window.innerHeight * 0.48);
    const x = isSmall ? 10 : window.innerWidth - w - 24;
    const y = window.innerHeight - h - 74;
    setWinSize({ w, h });
    setWinPos({ x, y });
    setWindowMode("split-50");
  }, []);
  ...
  useEffect(() => {
    const onWinResize = () => {
      ...
      const maxX = Math.max(0, window.innerWidth - winSize.w - 10);
      const maxY = Math.max(50, window.innerHeight - winSize.h - 70);
      setWinPos(p => p ? { x: Math.min(p.x, maxX), y: Math.min(p.y, maxY) } : null);
    };
    window.addEventListener("resize", onWinResize);
    return () => window.removeEventListener("resize", onWinResize);
  }, [winPos, winSize]);

  // Drag handling (Moving up, down, left, right anywhere)
  const handleDragStart = (e: React.MouseEvent | React.TouchEvent) => {
    if (windowMode === "maximized") return;
    const clientX = "touches" in e ? e.touches[0].clientX : e.clientX;
    const clientY = "touches" in e ? e.touches[0].clientY : e.clientY;
    
    // Ensure winPos and winSize are set
    const isSmall = window.innerWidth < 768;
    const curW = winSize?.w || (isSmall ? window.innerWidth - 20 : Math.min(500, window.innerWidth - 40));
    const curH = winSize?.h || Math.round(window.innerHeight * 0.48);
    const curX = winPos?.x ?? (isSmall ? 10 : window.innerWidth - curW - 24);
    const curY = winPos?.y ?? (window.innerHeight - curH - 74);
    ...
    const onMove = (moveEvent: MouseEvent | TouchEvent) => {
      ...
      const newX = Math.max(4, Math.min(window.innerWidth - curW - 4, initialX + deltaX));
      const newY = Math.max(50, Math.min(window.innerHeight - 90, initialY + deltaY));

      setWinPos({ x: newX, y: newY });
      if (!winSize) setWinSize({ w: curW, h: curH });
    };
  ...
  // Resize handling (Changing window size bigger/smaller in any direction)
  const handleResizeStart = (direction: "top" | "bottom" | "left" | "right" | "top-left" | "top-right" | "bottom-left" | "bottom-right", e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    if (windowMode === "maximized") return;
    const clientX = "touches" in e ? e.touches[0].clientX : e.clientX;
    const clientY = "touches" in e ? e.touches[0].clientY : e.clientY;
    ...
    const onMove = (moveEvent: MouseEvent | TouchEvent) => {
      ...
      if (direction.includes("top")) {
        newH = Math.max(180, Math.min(window.innerHeight - 100, initH - deltaY));
        newY = initY + (initH - newH);
      } else if (direction.includes("bottom")) {
        newH = Math.max(180, Math.min(window.innerHeight - initY - 70, initH + deltaY));
      }

      // Horizontal resizing
      if (direction.includes("left")) {
        newW = Math.max(260, Math.min(window.innerWidth - 16, initW - deltaX));
        newX = initX + (initW - newW);
      } else if (direction.includes("right")) {
        newW = Math.max(260, Math.min(window.innerWidth - initX - 8, initW + deltaX));
      }
  ...
  // Open App: Directly opens in Freeform Movable & Resizable Split Screen
  const openApp = (app: SmartAppItem) => {
    setDrawerOpen(false);

    // Only system dialer/sms if user explicitly opens phone/messages
    if (app.id === "phone") {
    ...
    // ALL OTHER APPS: Open in Movable & Resizable Split Dual Screen
    setActiveApp(app);
    ...
    <>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-[9990] pointer-events-none">
          ...
          {/* Pure White Icon Dock (Guaranteed clearance: 120px from top, 76px from bottom) */}
          <div className="absolute right-2 sm:right-3 top-[120px] bottom-[76px] w-16 sm:w-18 py-3 px-2 bg-white rounded-3xl border border-slate-200 shadow-2xl flex flex-col items-center gap-2 z-10 animate-in slide-in-from-right duration-200 pointer-events-auto">
            {/* Close Button */}
            <button
              onClick={() => setDrawerOpen(false)}
              className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-slate-700 flex items-center justify-center transition cursor-pointer active:scale-90 flex-shrink-0"
              title="Close"
            >
            ...
            <div className="flex flex-col items-center gap-2 overflow-y-auto scrollbar-hide py-1 flex-1 min-h-0 w-full">
              {userApps.map(app => (
                <button
                  key={app.id}
                  onClick={() => openApp(app)}
                  className="relative group cursor-pointer focus:outline-none p-2 rounded-2xl hover:bg-slate-100 active:scale-90 transition-all duration-150 flex items-center justify-center flex-shrink-0 mx-auto"
                  title={app.name}
                >
            ...
            <button
              ...
              className="w-9 h-9 rounded-2xl border border-dashed border-slate-300 hover:border-[#E05236] text-slate-400 hover:text-[#E05236] flex items-center justify-center transition-all duration-150 hover:bg-[#E05236]/5 active:scale-90 cursor-pointer flex-shrink-0"
      ...
      {/* ── 4. Split Screen / Freeform Movable & Resizable Mini Window ── */}
      {activeApp && !isMinimized && (
        <div
          className={`fixed z-50 flex flex-col bg-white shadow-2xl border border-slate-200/90 rounded-3xl overflow-hidden ${
          ...
          style={
            windowMode === "maximized"
              ? undefined
              : {
                  left: winPos ? `${winPos.x}px` : "10px",
                  top: winPos ? `${winPos.y}px` : "auto",
                  bottom: winPos ? "auto" : "66px",
                  width: winSize ? `${winSize.w}px` : "calc(100vw - 20px)",
                  height: winSize ? `${winSize.h}px` : `calc(${splitHeightPercent}vh - 40px)`,
                  maxWidth: "calc(100vw - 8px)",
                  maxHeight: "calc(100vh - 70px)",
                  transition: isInteracting ? "none" : "box-shadow 0.2s ease, border-radius 0.2s ease",
                  touchAction: "none"
                }
          }
        >
          {/* Top Edge Resize Handle (Drag height up/down) */}
          {windowMode !== "maximized" && (
            <div
              onMouseDown={(e) => handleResizeStart("top", e)}
              onTouchStart={(e) => handleResizeStart("top", e)}
              className="absolute top-0 left-12 right-12 h-3 cursor-ns-resize z-30 flex items-center justify-center -mt-0.5 group"
              title="Drag up/down to resize height"
            >
              <div className="w-12 h-1 rounded-full bg-white/40 group-hover:bg-white/90 transition" />
            </div>
          )}
          ...
          {windowMode !== "maximized" && (
            <div
              ...
              className="absolute top-10 bottom-10 left-0 w-2.5 cursor-ew-resize z-30 group"
          ...
          {windowMode !== "maximized" && (
            <div
              ...
              className="absolute top-10 bottom-10 right-0 w-2.5 cursor-ew-resize z-30 group"
              title="Drag left/right to resize width"
            />
          )}
          ...
          {windowMode !== "maximized" && (
            <div
              ...
              className="absolute bottom-0 left-12 right-12 h-2.5 cursor-ns-resize z-30 group"
              title="Drag to resize height"
            />
          )}

          {/* Window Top Drag Header Bar (Drag to move up, down, left, right) */}
          <div
            onMouseDown={handleDragStart}
            onTouchStart={handleDragStart}
            className="bg-slate-900 text-white flex flex-col select-none flex-shrink-0 cursor-move active:cursor-grabbing"
            title="Drag header to move window anywhere (Up/Down/Left/Right)"
          >
            {/* Split Screen Top Pill Handle with Height Quick Toggles (40% / 50% / 65%) */}
            <div className="pt-2 pb-0.5 flex items-center justify-center gap-2">
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  const nextPercent = splitHeightPercent === 50 ? 65 : splitHeightPercent === 65 ? 40 : 50;
                  setSplitHeightPercent(nextPercent);
                  const h = Math.round(window.innerHeight * (nextPercent / 100));
                  setWinSize(s => s ? { ...s, h } : null);
                  setWinPos(p => p ? { ...p, y: window.innerHeight - h - 74 } : null);
                }}
                className="flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-white/15 hover:bg-white/25 text-[10px] font-mono font-bold text-white transition active:scale-95 cursor-pointer"
                title="Click to toggle preset height: 40% | 50% | 65%"
              >
                <Move className="w-2.5 h-2.5 opacity-80" />
                <span>Move & Resize Dual Screen</span>
              </button>
            </div>
            ...
            <div className="px-3.5 py-1.5 flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0 pointer-events-none">
                <div className="w-6 h-6 rounded-lg bg-white/15 flex items-center justify-center text-white overflow-hidden p-0.5 flex-shrink-0">
                  {renderAppIcon(activeApp, "w-4 h-4 text-white")}
                </div>
                <span className="font-bold text-xs sm:text-sm truncate">
                  {activeApp.name}
                </span>
                <span className="text-[9px] font-mono uppercase px-1.5 py-0.5 rounded-sm bg-[#C04A22] text-white font-bold hidden sm:inline">
                  {windowMode === "maximized" ? "Full Screen" : "Freeform"}
                </span>
              </div>

              {/* Action Buttons (Stop event propagation so clicking them doesn't drag) */}
              <div
                onMouseDown={(e) => e.stopPropagation()}
                onTouchStart={(e) => e.stopPropagation()}
                className="flex items-center gap-1"
              >
                {/* Reset to Bottom 50% Split */}
                <button
                  type="button"
                  onClick={resetToBottomSplit}
                  className="px-2 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 bg-white/15 hover:bg-white/25 text-white/90 transition cursor-pointer"
                  title="Snap back to Bottom 50% Split"
                >
                  <Layers className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline">Dock 50%</span>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setWindowMode(m => m === "maximized" ? "split-50" : "maximized")}
                  className="w-7 h-7 rounded-lg hover:bg-white/20 flex items-center justify-center text-white/80 transition cursor-pointer"
                  title={windowMode === "maximized" ? "Restore Window" : "Full Screen"}
                >
                  {windowMode === "maximized" ? <Minimize2 className="w-3.5 h-3.5" /> : <Maximize2 className="w-3.5 h-3.5" />}
                </button>
                ...
                <button
                  ...
                  className="w-7 h-7 rounded-lg bg-red-500/80 hover:bg-red-500 flex items-center justify-center text-white transition cursor-pointer ml-0.5"
                  title="Close App"
                >
                  <X className="w-3.5 h-3.5" />
                </button>
          ...
          {/* Mini Window Direct Interactive App Execution Area (RUNS DIRECTLY IN OUR APP) */}
          <div className="flex-1 overflow-hidden flex flex-col bg-slate-50/50 relative">
            ...
            {activeApp.id === "facebook" && <FacebookMiniApp />}
            {activeApp.id === "instagram" && <InstagramMiniApp />}
            {activeApp.id === "bkash" && <BKashMiniApp />}
            ...
            {/* Any other app or custom user app runs in InAppWebViewer */}
            {!["youtube", "chatgpt", "google", "whatsapp", "facebook", "instagram", "bkash", "spotify", "camera", "photos", "voice_rec", "calculator", "clock", "notes_app", "translate", "uscis", "remittance", "wage_calc", "transit", "halal_finder"].includes(activeApp.id) && (
            ...
            {windowMode !== "maximized" && (
              <div
                ...
                className="absolute bottom-1 right-1 w-6 h-6 cursor-nwse-resize z-30 flex items-end justify-end p-1 text-slate-400 hover:text-slate-700 active:text-[#C04A22] transition-colors select-none"
            ...
            {windowMode !== "maximized" && (
              <div
                ...
                className="absolute bottom-1 left-1 w-6 h-6 cursor-nesw-resize z-30 flex items-end justify-start p-1 text-slate-400 hover:text-slate-700 active:text-[#C04A22] transition-colors select-none"
                title="Drag corner to resize width and height"
              >
                <svg className="w-3.5 h-3.5 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M3 15L9 21M3 8L16 21" />
                </svg>
              </div>
            )}
          </div>
          ...
          <div className="p-6 text-center space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-500 leading-relaxed max-w-xs mx-auto">
                Allow <strong>Pathasathi Smart Cloner</strong> to run apps inside Vivo-style floating mini windows?
              </p>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 text-[11px] text-slate-600 text-left space-y-1.5">
              <div className="flex items-center gap-2">
                ...
                <span>Runs directly in our PWA app inside floating mini screen</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />
                <span>Add &amp; customize any app with your own icon</span>
              </div>
            </div>
          ...
          <div className="p-8 text-center space-y-5 my-auto">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 font-mono">
                Preparing in-app mini windows ({scanProgress}%)
              </p>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Runs inside Vivo Mini Window • {userApps.length} in dock
                  </p>
                </div>
              </div>
              <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">{app.nameBn || "In-App Mini Screen"}</p>
                      </div>
                    </div>
...
// ── 1. YouTube Live Mini Window (Playable Video Engine inside Pathasathi) ─────
function YouTubeMiniApp() {
  const [currentVideoId, setCurrentVideoId] = useState("dQw4w9WgXcQ");
  const [searchQuery, setSearchQuery] = useState("");

  const YOUTUBE_FEEDS = [
    { id: "dQw4w9WgXcQ", title: "How to apply for Driver's License in NY (Bengali Guide)", channel: "Immigrant Compass USA", views: "142K views" },
    { id: "9bZkp7q19f0", title: "NYC Subway Map & Commuting Guide for Newcomers", channel: "NYC Transit Tips", views: "89K views" },
    { id: "kJQP7kiw5Fk", title: "Top 10 Bangladeshi Restaurants in Jackson Heights", channel: "Deshi Food Explorer", views: "230K views" },
    { id: "fJ9rUzIMcZQ", title: "US Citizenship Civics Test 100 Questions & Answers", channel: "USCIS Prep", views: "510K views" },
  ];
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchQuery.includes("youtube.com") || searchQuery.includes("youtu.be")) {
      const match = searchQuery.match(/(?:v=|\/embed\/|youtu\.be\/)([\w-]{11})/);
      if (match && match[1]) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0F0F0F] text-white">
      ...
      {/* Search Bar */}
      <form onSubmit={handleSearchSubmit} className="p-2.5 bg-[#181818] border-b border-white/10 flex items-center gap-2">
        ...
        <button type="submit" className="px-3 py-1.5 rounded-full bg-red-600 text-white text-xs font-bold cursor-pointer">
          Play
        </button>
      </form>
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2">
        <span className="text-[10px] font-bold uppercase text-slate-400 px-1">Suggested for you:</span>
        {filteredVideos.map(video => (
          <div
            key={video.id}
            onClick={() => setCurrentVideoId(video.id)}
            className={`p-2 rounded-2xl border transition flex gap-2.5 cursor-pointer active:scale-98 ${
              currentVideoId === video.id ? "bg-[#272727] border-red-500" : "bg-[#181818] border-white/5 hover:bg-[#222]"
            }`}
          >
            <div className="relative w-24 h-16 rounded-xl bg-slate-800 overflow-hidden flex-shrink-0 flex items-center justify-center">
...
// ── 2. WhatsApp In-App Messenger Mini Window ──────────────────────────────────
function WhatsAppMiniApp() {
  const [messages, setMessages] = useState([
    { id: 1, sender: "Community Helper", text: "Assalamu Alaikum! How can we assist you today in NY?", time: "10:30 AM", isMe: false },
    { id: 2, sender: "You", text: "I need help with Bangladeshi store locations.", time: "10:32 AM", isMe: true },
  ]);
  ...
  const handleSend = (e: React.FormEvent) => {
    e.preventDefault();
    if (!text.trim()) return;
    const newMsg = { id: Date.now(), sender: "You", text: text.trim(), time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), isMe: true };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0b141a] text-white">
      <div className="p-3 bg-[#202c33] flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center font-bold text-xs">
            WA
          </div>
          <div>
            <h4 className="font-bold text-xs">WhatsApp Mini Chat</h4>
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.isMe ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] p-2.5 rounded-2xl text-xs ${
              m.isMe ? "bg-[#005c4b] text-white rounded-br-none" : "bg-[#202c33] text-slate-100 rounded-bl-none"
            }`}>
              <p className="leading-relaxed">{m.text}</p>
              <span className="text-[9px] text-white/50 block text-right mt-1">{m.time}</span>
            </div>
          </div>
        ))}
      </div>

      <form onSubmit={handleSend} className="p-2.5 bg-[#202c33] flex items-center gap-2">
        <input
          type="text"
          value={text}
          onChange={e => setText(e.target.value)}
          placeholder="Type a message…"
          className="flex-1 bg-[#2a3942] rounded-xl px-3 py-2 text-xs text-white outline-none placeholder:text-slate-400"
        />
        <button type="submit" disabled={!text.trim()} className="w-8 h-8 rounded-xl bg-emerald-600 disabled:opacity-40 flex items-center justify-center text-white cursor-pointer">
...
// ── 3. Facebook In-App Social Mini Window ─────────────────────────────────────
function FacebookMiniApp() {
  const posts = [
    { author: "Bangladeshi Community NY", time: "2h ago", content: "🎉 Annual Community gathering in Queens this Saturday! All Bangladeshi families welcome." },
    { author: "Immigrant Connect News", time: "5h ago", content: "📋 USCIS announced new online filing options for immigrant workers." },
  ];

  return (
    <div className="flex-1 flex flex-col h-full bg-slate-100 text-slate-900">
      <div className="p-3 bg-[#1877F2] text-white flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-2 font-bold text-sm">
          <Facebook className="w-5 h-5 fill-white" />
          <span>Facebook Mini</span>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {posts.map((p, i) => (
          <div key={i} className="bg-white p-3 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-[#1877F2] text-white flex items-center justify-center text-xs font-bold">
                f
              </div>
              <div>
                <h5 className="font-bold text-xs leading-tight">{p.author}</h5>
                <span className="text-[10px] text-slate-400">{p.time}</span>
              </div>
            </div>
            <p className="text-xs text-slate-700 leading-relaxed">{p.content}</p>
            <div className="flex gap-4 pt-2 border-t border-slate-100 text-slate-500 text-[11px] font-bold">
              <button className="flex items-center gap-1 hover:text-blue-600"><Heart className="w-3.5 h-3.5" /> Like</button>
              <button className="flex items-center gap-1 hover:text-blue-600"><MessageSquare className="w-3.5 h-3.5" /> Comment</button>
            </div>
          </div>
        ))}
      </div>
...
// ── 4. Instagram In-App Reels & Photos Mini Window ────────────────────────────
function InstagramMiniApp() {
  const images = [
    { title: "Queens Sunset with Bengali friends", likes: "1.2K" },
    { title: "Halal Food Tour in Jackson Heights", likes: "3.4K" },
  ];

  return (
    <div className="flex-1 flex flex-col h-full bg-black text-white">
      <div className="p-3 bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 flex items-center justify-between text-white font-bold text-sm">
        <div className="flex items-center gap-2">
          <Instagram className="w-4 h-4" />
          <span>Instagram Mini</span>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {images.map((item, i) => (
          <div key={i} className="bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-800 space-y-2">
            <div className="h-44 bg-neutral-800 flex items-center justify-center text-neutral-500 text-xs">
              📸 Photo / Reel Stream
            </div>
            <div className="p-3 space-y-1">
              ...
              <p className="text-xs font-bold pt-1">{item.likes} likes</p>
              <p className="text-xs text-neutral-300">{item.title}</p>
            </div>
          </div>
        ))}
      </div>
...
// ── 5. bKash / Payment In-App Mini Window ─────────────────────────────────────
function BKashMiniApp() {
  const [amount, setAmount] = useState(100);
  const rate = 121.5;

  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 text-slate-900">
      <div className="p-3 bg-[#E2136E] text-white flex items-center justify-between">
        <span className="font-extrabold text-sm">bKash Remittance</span>
        <span className="text-xs font-mono font-bold">$1 = ৳121.50</span>
      </div>
      <div className="p-4 space-y-3 flex-1 overflow-y-auto">
        <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
          <label className="text-[11px] font-bold text-slate-500 block uppercase">Send USD from USA</label>
          <input
            type="number"
            value={amount}
            onChange={e => setAmount(Number(e.target.value))}
            className="w-full text-lg font-bold p-2 bg-slate-50 border border-slate-200 rounded-xl outline-none"
          />
        </div>

        <div className="bg-[#FFF0F6] p-3.5 rounded-2xl border border-[#E2136E]/20 text-center space-y-1">
          <span className="text-xs text-[#E2136E] font-bold">Recipient receives in Bangladesh bKash:</span>
          <div className="text-2xl font-extrabold text-[#E2136E] font-mono">
            ৳{(amount * rate).toLocaleString()} BDT
          </div>
        </div>

        <button onClick={() => alert("Connecting to bKash Direct Gateway...")} className="w-full py-3 bg-[#E2136E] text-white rounded-2xl font-extrabold text-xs shadow-md cursor-pointer active:scale-95">
          Send Money to bKash
        </button>
      </div>
    </div>
...
// ── 6. Spotify Music Player Mini Window ───────────────────────────────────────
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#121212] text-white p-4 justify-between">
      <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs">
        ...
        <span>Spotify Mini Player</span>
      </div>

      <div className="text-center space-y-3">
        <div className="w-32 h-32 mx-auto rounded-2xl bg-neutral-800 border border-neutral-700 flex items-center justify-center">
          <Music className="w-12 h-12 text-emerald-400" />
        </div>
        <div>
          <h4 className="font-bold text-sm">Bengali Community Favorites</h4>
          <p className="text-xs text-neutral-400">Pathasathi Radio Station</p>
        </div>
      </div>

      <div className="space-y-2">
        <div className="flex justify-center items-center gap-4">
          <button onClick={() => setPlaying(p => !p)} className="w-12 h-12 rounded-full bg-emerald-500 text-black flex items-center justify-center font-bold cursor-pointer">
            {playing ? <Pause className="w-5 h-5" /> : <Play className="w-5 h-5 fill-black" />}
          </button>
        </div>
      </div>
    </div>
...
// ── 9. In-App Web Viewer for Other Apps ───────────────────────────────────────
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white">
      <div className="p-2 border-b border-slate-200 bg-slate-100 flex items-center justify-between gap-2 text-xs">
        ...
        <a href={targetUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 transition" title="Open Full View">
